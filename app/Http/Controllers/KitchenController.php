...
class KitchenController extends Controller
{
    ...
    { 
      ...
      $addons = DB::table('kitchen_customize')->join('kitchen_item_addons','kitchen_customize.id','=','kitchen_item_addons.order_id')->get();
    ...
    {	
      ...
      {   
        if($ifexist == null)
        {
          ...
          $id = DB::table('kitchen')->where('table_number',$table_number)->where('item_id',$request->item_id)->where('confirm_status',null)->first();
        ...
        $id = DB::table('kitchen_customize')->orderBy('id','DESC')->first();
    ...
    public function customize(Request $request){
      ...
      if($request->action == 'add'){
        ...
        $sum = DB::table('kitchen_customize')->where('order_id',$get_id[0])->get()->sum('quantity');
        DB::table('kitchen')->where('id',$get_id[0])->where('confirm_status',null)->update(['item_quantity' => $sum]);
        $total_items = DB::table("kitchen")->where("table_number","=",$table_number)->where('confirm_status',null)->get()->sum("item_quantity");
      ...
      elseif ($request->action == 'minus') {
        ...
        if($update->quantity == '0'){
          DB::table('kitchen_customize')->where('order_id',$update->id)->delete();
          $update->delete();
          DB::table('kitchen_item_addons')->where('order_id',$request->item_id)->delete();
          $check_main_item = Kitchen::where('table_number',$table_number)->where('confirm_status',null)->where('id',$get_id[0])->pluck('item_quantity');
          $item_id_to_change = Kitchen::where('table_number',$table_number)->where('confirm_status',null)->where('id',$get_id[0])->pluck('item_id');
          if($check_main_item[0] == 0){
            DB::table('kitchen')->where('confirm_status',null)->where('id',$get_id[0])->delete();
          }
          $response = array(
      ...
      $kitchen_addon = DB::table('kitchen_customize')->join('kitchen_item_addons','kitchen_customize.id','=','kitchen_item_addons.order_id')->get();
      $total_items = DB::table("kitchen")->where('confirm_status',null)->where("table_number","=",$table_number)->get()->sum("item_quantity");
      $response = array(
        'status' => 'success',
